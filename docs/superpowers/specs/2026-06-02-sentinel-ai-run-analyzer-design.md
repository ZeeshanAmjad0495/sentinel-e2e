...
`redactEvents(events): TelemetryEvent[]` — defense-in-depth before anything is sent to the API. Deep-clones events and replaces values whose KEY matches a secret pattern (`/pass(word)?|secret|token|api[-_]?key|authorization|cookie|credential/i`) with `"[redacted]"`, and replaces any string value that looks like a high-entropy secret/JWT. The telemetry is already credential-free (verified in slice A), so this is belt-and-suspenders; it is unit-tested with a planted secret field to prove the payload reaching a (spy) provider is clean. **The orchestrator sends only redacted telemetry events — never `process.env` or any other host data.**
