...
- **Scope:** Make the five framework packages installable like Playwright (`npm i @sentinele2e/core` etc.) — real `dist` builds, `exports`/`main`/`types` pointing at compiled output, `files`/`publishConfig`, consistent versions, concrete cross-dep ranges — **publish-ready and `npm pack`-verified** (actual `npm publish` needs registry credentials, out of scope). The dev/test path-alias resolution (`@sentinele2e/*` → `src` via tsconfig) is preserved unchanged, so the offline test suite stays green.
...
| D-2 | **Dual resolution: published→`dist`, dev/test→`src`**                                                                                                           | `package.json` `main`/`types`/`exports` point at `dist` (what installed consumers get). The repo's `tsconfig.base.json` `paths` keep mapping `@sentinele2e/*` → `src` for the Playwright unit-runner + `tsc -b`, so tests/typecheck are unaffected. This is the standard library dual-entry; the two never conflict because the test loader uses tsconfig paths, not `package.json main`. |
...
- **Cross-deps:** replace `"@sentinele2e/contracts": "*"` / `"@sentinele2e/core": "*"` with `"^0.1.0"`. (npm workspace install still symlinks locally because the local version `0.1.0` satisfies `^0.1.0`.)
- **`@sentinele2e/ai`** keeps `"bin": { "sentinel-analyze": "dist/cli.js" }` (already correct); ensure `dist/cli.js` ships (covered by `files:["dist"]`) and has a shebang (`#!/usr/bin/env node`) — add the shebang to `src/cli.ts` if absent (tsc preserves leading shebang).
...
- Add `"pack:check": "npm run build && for p in contracts core driver-playwright driver-selenium ai; do npm pack -w @sentinele2e/$p --dry-run; done"` (or a small node script) — lists the tarball contents for verification.
...
`tsconfig.base.json` `paths` (`@sentinele2e/* → packages/*/src/...`) are **unchanged**. The Playwright unit runner and `tsc -b` resolve via those paths → `src`. Therefore:
...
- The `analyze` script (`tsc -b && node packages/ai/dist/cli.js`) still works — it builds dist then runs the compiled CLI, which resolves its `@sentinele2e/*` deps at runtime via `package.json main → dist` (now that those exist post-build).
...
3. For each publishable package, `npm pack -w @sentinele2e/<pkg> --dry-run` lists ONLY `dist/**` + `package.json` + `README.md` (+ `LICENSE` if present) — **no `src/`, `tests/`, `tsconfig*`**.
4. **Install-verify (best-effort):** `npm run build` then in a temp dir, `npm pack` the tarballs and `npm i` them into a throwaway project; `node -e "require('@sentinele2e/core')"` and `require('@sentinele2e/contracts')` resolve from `dist` without error; `npx sentinel-analyze <a sample jsonl>` runs from the installed `ai` package. (If the sandbox can't install cross-package tarballs cleanly, at minimum prove each tarball's `main` file exists in the tarball and `node -e "require('<unpacked>/dist/index.js')"` loads.)
